...
function Album({ path, setPath }) {
  ...
  /* Layout:
     - Desktop: two-pane (sidebar + viewer)
     - Mobile: single column (sidebar full width). Tap thumbnail opens in new tab.
  */
  return (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        >
          {photos.map((p) => {
            const isActive = !isMobile && selected && selected.key === p.key;
            return (
              <button
                key={p.key}
                onClick={() => {
                  if (isMobile) {
                    window.open(p.fullUrl, "_blank", "noopener,noreferrer");
                  } else {
                    setSelected(p);
                    setZoom(1);
                  }
